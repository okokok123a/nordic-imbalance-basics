...
def build_demo(area: str, start: str, end: str) -> pd.DataFrame:
    ...
    idx = pd.date_range(
        start=start, end=end, freq="H", tz="Europe/Stockholm", inclusive="left"
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "imbalance_volume_mwh": volume_mwh.astype(float),
        },
        index=idx,
    )
    df.index.name = "ts"
...
def main():
    ap = argparse.ArgumentParser(
        description="Fetch Nordic imbalance prices/volumes (demo-supported)."
    )
    ...
    ap.add_argument(
        "--demo",
        action="store_true",
        help="Generate synthetic data instead of fetching",
    )
    args = ap.parse_args()
    ...
    if args.demo:
        ...
        raise NotImplementedError(
            "Real eSett/ENTSO-E fetch coming next. Use --demo for now."
        )
